...
端到端用户、数据、审核与发布流程见 [`END_TO_END_OPERATING_MODEL.md`](END_TO_END_OPERATING_MODEL.md)。
...
- 在线预发布：Cloudflare Workers 前端 + Render Free FastAPI + Cloudflare Cron + Neon PostgreSQL；Cron 每 30 分钟调用受保护的单周期接口并对 Render 冷启动执行有限重试，GitHub Actions 只保留 `workflow_dispatch` 手动直连 Neon 的应急入口。
...
- 当前自动抽取只生成候选，所有关系和普通事实均进入人审；是否开放任何自动批准必须另行授权并由真实精度证明，不以数据增长速度换取可信度。
- Cloudflare Cron 使用独立自动化令牌、有限重试、结构化生命周期日志和数据库租约；GitHub 手动应急任务使用仓库级 Neon Secret。运维诊断只记录信源 ID，不把 URL 参数、错误正文或凭据写入外部日志。

...
- 已部署后端提交：`9cc7a9c38c6e08559c5ea0159e36a0f2ef224c3e`
- 已部署前端提交：`e049361`
- 数据模式：`demo`
...
| -------------- | -------------------------------------------------------------------------- |
| Git HEAD       | `9cc7a9c`                                                                  |
| GitHub Quality | 当前 HEAD 运行失败；后端通过，前端格式检查失败                             |
| Render         | `/ready` 返回 200，commit `9cc7a9c`，schema `20260901_0022`                |
| Cloudflare     | `/version.txt` 为 `e049361`，与后端不一致                                  |
...
| 审核           | open 32 / approved 195 / rejected 397                                      |
...
2. 本地 Blueprint 已将普通自动抽取上限恢复为 0，但尚未部署并核验运行时开关。
3. 本地 Quality 等价门禁已绿色，但 GitHub 检查尚未由本次提交触发。
4. Cloudflare 与 Render 仍不在同一提交；staging 发布门禁代码已准备，尚未执行部署和 smoke。
5. 本地产品闭环已补齐，但登录态实时研究、发布分享和完整 staging 用户流程仍需在同一提交部署后验收。
6. 数据质量门禁仍未通过，不能切换 `live`。
7. 当前部署目标仍是 staging；`main` 分支与正式 production 发布方式尚未定义。
...
- [ ] 前后端完整门禁绿色，部署流程只在所有检查通过后启动。
- [ ] Render 与 Cloudflare 运行同一已验证提交。
...
| 节点   | 目标                          | 当前状态                                      | 可交付证据                                     |
| ------ | ----------------------------- | --------------------------------------------- | ---------------------------------------------- |
| Node 0 | 安全收口与停止未授权抽取      | 旧访问令牌已失效且本地已清理；待部署核验开关  | 旧凭证失效证明、tip 清理、运行时开关响应       |
| Node 1 | 固化当前代码与 UI 基线        | 本地门禁与桌面/390px 验收通过；待整理提交边界 | 完整本地门禁、桌面与真实 390px 验收、有限 diff |
| Node 2 | 让 CI 和 staging 运行同一提交 | CI/发布门禁代码已准备；尚未提交部署           | 绿色 Quality、两端完整 commit、smoke 记录      |
| Node 3 | 收口开放审核和关系候选        | 默认只读                                      | 逐条决定记录、操作前后计数、质量报告           |
| Node 4 | 交付完整产品闭环              | 本地契约与主流程已实现；待 staging 登录态验收 | 页面级验收、交互测试、引用链路和任务测试       |

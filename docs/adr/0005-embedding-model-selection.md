...
4. Embedding 使用独立 `evidence_embedding` 表，而不是在 Evidence 表上增加固定维度列。独立表必须保存 provider、模型、版本、维度、内容哈希、向量和生成时间，并以 Evidence 与模型版本形成唯一约束。

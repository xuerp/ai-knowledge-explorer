...
- 当前待处理 **44** 条：新鲜安全 0，确定性无效 44。下一个写操作是拒绝这 44 条无效库存，需要在执行前单独确认。
...
在确认当前 44 条均可放弃后，以标准化 `schema_error` 原因拒绝确定性无效项，并保留审计记录。

- 优点：最快消除无效库存，不会把无证据事实发布出去。
- 代价：可能丢失可通过补证恢复的候选；拒绝后若要恢复，需要重新生成或重新入队。
- 授权粒度：必须明确批准整组 44 条，并在写前重新读取版本。

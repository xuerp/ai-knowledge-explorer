# Staging 审核库存只读盘点（2026-09-08）

## 结论

E0 运行时安全核验和 E1 审核库存分级已完成。当前 34 条开放候选全部未达到批准条件，批准就绪数为 **0**；本次没有修改候选、调用模型或执行批量审核。

建议暂时保持 34 条为 open。下一步应先决定是保守拒绝确定性无效项，还是补齐实体绑定与 Evidence 原文锚点后重新审核。两种路径都属于线上写操作，必须获得当次明确授权。
...
在确认 34 条均可放弃后，以有限批次和标准化 `schema_error` 原因拒绝确定性无效项，并保留审计记录。

- 优点：最快消除无效库存，不会把无证据事实发布出去。
- 代价：可能丢失可通过补证恢复的候选；拒绝后若要恢复，需要重新生成或重新入队。
- 授权粒度：必须明确具体 ID 或明确批准整组 34 条，并在写前重新读取版本。

### 方案 B：修复后重审（推荐）

先补齐实体绑定和可定位的 Evidence 原文锚点，再重新进行重复、更新、冲突与风险判断；只有通过全部校验的项目才进入批准批次。

- 优点：最大限度保留有价值内容，形成可追溯事实链。
- 代价：需要更多人工核验和受控写入；若需要重新抓取或调用模型，必须另行授权预算。
- 建议顺序：先处理 1 条缺实体项，再按来源域分批修复 33 条无效锚点，每批不超过 5 条。

在用户选择并授权前，不执行“清理确定性无效”、批准、拒绝、合并、采集或抽取。
